import { Injectable, BadRequestException, NotFoundException, UseGuards } from '@nestjs/common';
...
export class BookingsService {
    ...
    async createBooking(userId: string, dto: CreateBookingDto) {
        ...
        const fullBooking = await this.prisma.booking.findUnique({
            ...
            include: {
                ...
                Payment: {
                    select: { id: true, stripePaymentId: true, amount: true, status: true }
                }
            }
        })
    ...
    async handleStripeWebhook(rawBody: Buffer, sig: string, endpointSecret: string) {
        ...
        if (event.type === 'payment_intent.succeeded') {
            ...
            if (bookingId) await this.prisma.booking.update({ where: { id: bookingId }, data: { status: 'COMPLETED', paymentId: pi.id } });
        }
